...
class IsRolePermission(IsValidUserPermission):
    ...
    def has_permission(self, request, view):
        is_role = (
            ...
            and IsValidUserPermission.has_permission(self, request, view)
        )
        if not is_role:
            return False
...
class CanCrudPermission(permissions.BasePermission):
        ...
        if view.action == "create" or is_detail_request:
            ...
            for fields in view.model._meta.get_fields():
                ...
                if fields.is_relation:
                    if not fields.related_model.raw_can_modify(
                        request.user, request.data[fields.name]
                    ):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if view.action in ["retrieve"]:
            return (
                obj.can_modify(request.user, [])
                or obj.can_read(request.user)
                or obj.is_public()
            )
