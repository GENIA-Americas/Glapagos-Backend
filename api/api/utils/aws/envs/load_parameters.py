...
def aws_ssm_parameter_serialize(path, param):
    ...
    if param["Type"] == "SecureString":
        print(
            "AWS SSM PARAMETER STORE SecureString variables serialization not implemented yet"
        )
        return name, param["Value"]
...
def get_params_from_ssm(path, parameter_names, region):
    ...
    for offest in range(0, len(parameter_names), 10):
        new_params = get_params_batch_from_ssm(
            client, path, parameter_names[offest : offest + 10]
        )
