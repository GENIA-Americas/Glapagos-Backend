...
class BaseModel(models.Model):
    ...
    deleted = models.BooleanField(
        default=False, help_text="Set to False when an element is deleted"
    )
    ...
    def proper_created(self):
        ...
        return self.created.astimezone(settings_time_zone).strftime(
            "%d %B, %Y %I:%M %p"
        )
    ...
    def proper_modified(self):
        settings_time_zone = pztimezone(settings.TIME_ZONE)
        return self.modified.astimezone(settings_time_zone).strftime(
            "%d %B, %Y %I:%M %p"
        )

    @property
    def is_modified(self):
        if self.modified.strftime("%d %B, %Y %I:%M %p") == self.created.strftime(
            "%d %B, %Y %I:%M %p"
        ):
            return None
        settings_time_zone = pztimezone(settings.TIME_ZONE)
    ...
    def raw_can_modify(self, *args, **kwargs):
        return True
