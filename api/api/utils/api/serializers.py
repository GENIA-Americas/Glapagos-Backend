...
def django_serializers_from_model_field(related_field):
    ...
    return django_serializers_from_model(
        related_field.field.model, fields=serializer_fields
    )
...
def django_serializer_from_model_definition(
    model_definition,
    crud_serializers={},
    ...
    related_fields_expand = get_key_chain(
        model_definition, ["actions", "read", "related_fields_expand"], []
    )
        ...
        for related_field in related_fields_expand:
            if hasattr(model_class, related_field):
                ...
                serializer_read_fields[related_field] = (
                    django_serializers_from_model_field(
                        getattr(model_class, related_field)
                    )["list"]
                )
    ...
    if requester_field:
        for field_name in serializer_field_names:
            if field_name in requester_field:
                serializer_write_fields[field_name] = serializers.HiddenField(
