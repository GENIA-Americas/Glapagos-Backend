...
from api.utils.api.model import model_to_model_defintion
...
def get_serializer_class_function(model_definitions, serializer_classes=None):
    ...
    def get_serializer_class(self, *args, **kwargs):
        if self.action in serializer_classes:
            ...
            and "write" in serializer_classes
        ):
...
def model_viewset_from_model_definition(
    serializer_classes,
    model_definition,
    permissions=None,
    lookup_field=None,
    ...
    viewset_class_attributes = {
        # "__module__": "cmsapi.api.views",
        "queryset": model_class.objects.all().order_by("id"),
        "permission_classes": (
            permissions
            if permissions
            else get_viewset_permissions_by_actions(action_definitions)
        ),
        "get_serializer_class": get_serializer_class_function(
            model_definition, serializer_classes=serializer_classes
        ),
    }
...
def crud_from_model(
    ...
    model_definitions = model_to_model_defintion(model)
    model_definitions["actions"] = {
        "crud": {},
    }
    if default_serializer:
        serializer_classes = {"crud": default_serializer}
    else:
        serializer_classes = django_serializer_from_model_definition(
            model_definitions, requester_field=requester_field
        )
...
def read_view_from_model(
    ...
    model_definitions = model_to_model_defintion(model, fields)
    model_definitions["actions"] = {
        "read": {
    ...
    serializer_classes = (
        ...
        else django_serializer_from_model_definition(model_definitions)
    )
    viewset = model_viewset_from_model_definition(
        serializer_classes,
        model_definitions,
