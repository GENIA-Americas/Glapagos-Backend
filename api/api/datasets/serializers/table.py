    ...
    def validate_convert_to(self, value):
        ...
        if value.upper() not in data_types:
            raise serializers.ValidationError(
                {"detail": _("Invalid format to convert")}
            )
        return value.upper()
...
class SingleTransformSerializer(serializers.Serializer):
    ...
    transformation = serializers.ChoiceField(
        choices=[(tag.value, tag.name) for tag in TransformationOption]
    )
    ...
    def validate(self, attrs):
        ...
        if transformation == TransformationOption.DATA_TYPE_CONVERSION.value and (
            ...
            raise serializers.ValidationError(
                _(
                    "'{option}' is a mandatory field for {transformation} transformation"
                ).format(option="convert_to", transformation=transformation)
            )
            ...
            raise serializers.ValidationError(
                _(
                    "'{option}' is a mandatory field for {transformation} transformation"
                ).format(option="text_case", transformation=transformation)
            )
        return attrs
    ...
    def validate(self, attrs):
        ...
        if not is_owner:
            if not table.public:
                raise serializers.ValidationError(
                    _(
                        "You do not have permission to transform this table because you are not the owner and the table is not public."
                    )
                )
            elif not create_table:
                raise serializers.ValidationError(
                    _(
                        "You do not have permission to modify the original table, and creating a new table has not been requested."
                    )
                )
        ...
        if create_table and public_destination is None:
            raise serializers.ValidationError(
                _(
                    "If you want to create a new table, you must specify its privacy setting."
                )
            )
...
class TableSchemaSerializer(serializers.Serializer):
    ...
    def validate_field(self, value):
        ...
        raise serializers.ValidationError(
            _("Field '{field}' does not exist in the dataset schema").format(
                field=value
            )
        )
