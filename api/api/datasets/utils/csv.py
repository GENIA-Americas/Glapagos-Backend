...
def prepare_csv_data_format(data: str, skip_leading_rows: int) -> List:
    ...
    for column in df.columns:
        ...
        result.append(
            {
                "column_name": (
                    normalize_column_name(column) if skip_leading_rows > 0 else None
                ),
                "data_type": bigquery_type,
                "mode": mode,
                "example_values": df[column][first_example_index:]
                .head(5)
                .fillna("")
                .tolist(),
            }
...
def get_content_from_url_csv(
    urls: list[str], max_lines: int | None = 20, skip_leading_rows: int = 1, **kwargs
) -> str:
    """
...
def validate_csv_column_names(df: pd.DataFrame, raise_exception=False) -> list:
    ...
    if invalid_columns and raise_exception:
        raise InvalidCsvColumnException(
            detail=_("Invalid column names in CSV:")
