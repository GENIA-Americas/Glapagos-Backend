...
def send_sms_twilio(phone_number, message, language_code=None):
    ...
    message = client.messages.create(
        to=phone_number, from_=settings.TWILIO_FROM_PHONE_NUMBER, body=message
    )


SMS_PROVIDERS = dict(twilio=send_sms_twilio)
...
def send_sms_message(phone_number, message, language_code=None):
    ...
    SMS_PROVIDERS[settings.SMS_PROVIDER](
        phone_number=phone_number, message=message, language_code=language_code
    )
