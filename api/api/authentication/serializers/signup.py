...
class SignUpValidateCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user_queryset:
            raise serializers.ValidationError({"detail": _("user is invalid")})
...
class ForgotPasswordRequestCodeSerializer(serializers.Serializer):
    ...
    channel = ChoiceField(
        choices=ExternalTokenChannel.choices, default=ExternalTokenChannel.CONSOLE
    )
...
class SignUpEmailSerializer(serializers.Serializer):
    ...
    country = serializers.ChoiceField(
        required=False, allow_blank=True, choices=Country.choices
    )
    country_code = serializers.RegexField(
        regex=r"^\+?[0-9]{1,3}([- ]?[0-9]{1,3})?$",
        max_length=10,
        error_messages={"invalid": _("Invalid country code.")},
    )
    phone_number = serializers.RegexField(
        regex=r"^\d+$",
        max_length=16,
        error_messages={"invalid": _("The phone number must contain only digits.")},
    )
    organization = serializers.CharField(required=False, allow_blank=True)
    industry = serializers.ChoiceField(
        required=False, allow_blank=True, choices=Industry.choices
    )
